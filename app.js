...
var widthloc_electric;
var heightloc_electric;
var nchargesloc;
...
function animate()
{
    ...
    gl.uniform1f(widthloc_electric, table_width);
    gl.uniform1f(heightloc_electric, table_height);
    gl.drawArrays(gl.LINES, 0, eletric_point.length);
...
function setup(shaders)
{
    ...
    widthloc_electric = gl.getUniformLocation(program_eletric_field, "table_width");
    heightloc_electric = gl.getUniformLocation(program_eletric_field, "table_height");
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, pBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, eletric_point.length*MV.sizeof["vec2"], gl.STATIC_DRAW)
    gl.bufferSubData(gl.ARRAY_BUFFER, 0, MV.flatten(eletric_point));

    const vPositionElectric = gl.getAttribLocation(program_eletric_field, "vPosition");
    gl.vertexAttribPointer(vPositionElectric, 2, gl.FLOAT, false, 0, 0);
    gl.enableVertexAttribArray(vPositionElectric);
